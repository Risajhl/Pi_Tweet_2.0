...
import ir.pi.project.controller.myPage.lists.followers.FollowersComponentLogic;
...
public class FollowersListPageController implements Initializable {
    @FXML
    ScrollPane scroller;
    ...
    public void update(List<Integer> followers) {
        try {
            ...
            for (int i = 0; i < followers.size(); i++) {
                FXMLLoader fxmlLoader=new FXMLLoader(MainController.class.getResource(fxmlConfig.getFollowersComponent()));
                AnchorPane anchorPane=fxmlLoader.load();
                content.getChildren().add(anchorPane);
                new FollowersComponentLogic(fxmlLoader.getController(),followers.get(i),content,myPagePageController);
            }
        }
        catch (Exception e){
            e.printStackTrace();
